...
vec pack109final::serialize(bool item) {
  vec bytes;
  if (item == true) {
    bytes.push_back(PACK109FINAL_TRUE);
  } else {
    bytes.push_back(PACK109FINAL_FALSE);
  }
  return bytes;
}

bool pack109final::deserialize_bool(vec bytes) {
  ...
  if (bytes[0] == PACK109FINAL_TRUE) {
    return true;
  } else if (bytes[0] == PACK109FINAL_FALSE) {
    return false;
  } else {
...
vec pack109final::serialize(u8 item) {
  vec bytes;
  bytes.push_back(PACK109FINAL_U8);
  bytes.push_back(item);
  return bytes;
}

u8 pack109final::deserialize_u8(vec bytes) {
...
vec pack109final::serialize(u32 item) {
  vec bytes;
  bytes.push_back(PACK109FINAL_U32);
...
u32 pack109final::deserialize_u32(vec bytes) {
...
vec pack109final::serialize(u64 item) {
  vec bytes;
  bytes.push_back(PACK109FINAL_U64);
  u64 mask = MASK8;
  for(int i = 7; i >= 0; i--) {
    int to_shift = 8 * i;
...
u64 pack109final::deserialize_u64(vec bytes) {
  if (bytes.size() < 9) {
    throw;
  }
  u64 deserialized_u64 = 0; // This is a u64 now, so we can fit all the bytes
  if (bytes[0] == PACK109FINAL_U64) {
    int ix = 1;
    for(int i = 7; i >= 0; i--) {
      int to_shift = 8 * i;
...
vec pack109final::serialize(i8 item) {
  vec bytes;
  bytes.push_back(PACK109FINAL_I8);
  // We can cast the i8 to a u8 and push because it's just a byte
  bytes.push_back((u8)item); 
  return bytes;
}

i8 pack109final::deserialize_i8(vec bytes) {
  if (bytes.size() < 2) {
    throw;
  }
  if (bytes[0] == PACK109FINAL_I8) {
...
vec pack109final::serialize(i32 item) {
  vec bytes;
  bytes.push_back(PACK109FINAL_I32);
  i32 mask = MASK4; 
  for(int i = 3; i >= 0; i--) {
    i32 to_shift = 8 * i;
...
i32 pack109final::deserialize_i32(vec bytes) {
  if (bytes.size() < 5) {
    throw;
  }
  i32 deserialized_i32 = 0;
  if (bytes[0] == PACK109FINAL_I32) {
    int ix = 1;
    for(int i = 3; i >= 0; i--) {
      int to_shift = 8 * i;
...
vec pack109final::serialize(i64 item) {
  vec bytes;
  bytes.push_back(PACK109FINAL_I64);
  i64 mask = MASK8;
  for(int i = 7; i >= 0; i--) {
    int to_shift = 8 * i;
...
i64 pack109final::deserialize_i64(vec bytes) {
  if (bytes.size() < 9) {
    throw;
  }
  i64 deserialized_i64 = 0; 
  if (bytes[0] == PACK109FINAL_I64) {
    int ix = 1;
    for(int i = 7; i >= 0; i--) {
      int to_shift = 8 * i;
...
vec pack109final::serialize(f32 item) {
  ...
  vec bytes;
  bytes.push_back(PACK109FINAL_F32);
  u32 mask = MASK4;
  for(int i = 3; i >= 0; i--) {
    int to_shift = 8 * i;
...
f32 pack109final::deserialize_f32(vec bytes) {
  if (bytes.size() < 5) {
    throw;
  }
...
f64 pack109final::deserialize_f64(vec bytes) {
  if (bytes.size() < 9) {
    throw;
  }
  u64 deserialized_u64 = 0;
  if (bytes[0] == PACK109FINAL_F64) {
    int ix = 1;
    for(int i = 7; i >= 0; i--) {
      int to_shift = 8 * i;
      u64 shifted = (u64)bytes[ix] << to_shift;
...
vec pack109final::serialize(string item) {
  vec bytes;
  if (item.size() < 256) {
    bytes.push_back(PACK109FINAL_S8);
    ...
    bytes.push_back(PACK109FINAL_S16);
...
string pack109final::deserialize_string(vec bytes) {
  if(bytes.size() < 3) {
    throw;
  }
  string deserialized_string = "";
  if(bytes[0] == PACK109FINAL_S8) {
  ...
  else if(bytes[0]==PACK109FINAL_S16) {
...
vec pack109final::serialize(std::vector<u8> item) {
  vec bytes;
  if (item.size() < X8) {
    bytes.push_back(PACK109FINAL_A8);
    ...
    bytes.push_back(PACK109FINAL_A16);
...
vec pack109final::serialize(std::vector<u64> item) {
  vec bytes;
  if (item.size() < X8) {
    bytes.push_back(PACK109FINAL_A8);
    u8 size = (u8)item.size();
...
vec pack109final::serialize(std::vector<f64> item) {
  vec bytes;
  if (item.size() < X8) {
    bytes.push_back(PACK109FINAL_A8);
    u8 size = (u8)item.size();
...
vec pack109final::serialize(std::vector<string> item) {
  vec bytes;
  if (item.size() < X8) {
    bytes.push_back(PACK109FINAL_A8);
    u8 size = (u8)item.size();
...
std::vector<u8> pack109final::deserialize_vec_u8(vec bytes) {
  if(bytes.size() < 3) {
    throw;
  }
  int el_size = 2;
  std::vector<u8> result;
  if(bytes[0] == PACK109FINAL_A8) {
  ...
  } else if(bytes[0] == PACK109FINAL_A16) {
...
std::vector<u64> pack109final::deserialize_vec_u64(vec bytes) {
  if(bytes.size() < 3) {
    throw;
  }
  int el_size = 9;
...
std::vector<f64> pack109final::deserialize_vec_f64(vec bytes) {
  if(bytes.size() < 3) {
    throw;
  }
  int el_size = 9;
...
vec pack109final::serialize(struct Person item) {
  vec bytes;
  bytes.push_back(PACK109FINAL_M8);
...
vec pack109final::serialize(struct File item){
    vec bytes; 
    bytes.push_back(PACK109FINAL_M8); 
    bytes.push_back(0x1); 
...
vec pack109final::serialize(struct Request item){
  vec bytes; 
  bytes.push_back(PACK109FINAL_M8); 
  bytes.push_back(0x1); 
...
struct Person pack109final::deserialize_person(vec bytes) {
  if (bytes.size() < 10) {
    throw;
  }
...
struct File pack109final::deserialize_file(vec bytes){
  ...
  struct File file; 
  string name; 
  for(int i = 0; i< (int)bytes.at(17); i++){
    name.push_back(bytes.at(18+i)); 
  }
  file.name = name; 
  vec hold; 
  if(bytes.at(25 + (int)bytes.at(17)) == 0xac){
    for(int i = 0; i < (int)bytes.at(26 + (int)bytes.at(17)); i++){
      hold.push_back(bytes.at(28 + (int)bytes.at(17) + i*2)); 
    }
  }else if(bytes.at(25 + (int)bytes.at(17)) == 0xad){
    short t = bytes.at(26 + (u8)bytes.at(17)); 
    t = (t<<8) | ((short)bytes.at(27 + (int)bytes.at(17)) & 0x00FF); 
    for(int i = 0; i < t; i++){
      hold.push_back(bytes.at(29 + (int)bytes.at(17) + i*2)); 
    }
  }else{
    throw; 
  }
...
struct Request pack109final::deserialize_request(vec bytes) {
    struct Request result;
    unsigned int i = 2;
    string pair_1_key_1 = pack109final::deserialize_string(vec(bytes.begin() + i, bytes.end()));
    i += (2 + pair_1_key_1.size() + 2);
    string pair_2_key_1 = pack109final::deserialize_string(vec(bytes.begin() + i, bytes.end()));
    i += (pair_2_key_1.size() + 2);
    string filename = pack109final::deserialize_string(vec(bytes.begin() + i, bytes.end()));
    result.name = filename;
    return result;
}


void pack109final::printVec(vec &bytes) {
  printf("[ ");
